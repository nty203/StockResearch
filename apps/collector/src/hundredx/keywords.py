...
GLOBAL_MEGA_KEYWORDS = ["방산", "원전", "SMR", "nuclear", "defense", "NATO",
                         ...
                         "방위산업", "무기 수출", "천무", "비궁", "신궁", "해성", "K방산",
                         "LIG넥스원", "한화에어로스페이스", "현대로템",
                         # 원전 세부
                         ...
                         "AEG", "BTM 전력", "가스엔진 수주", "4행정 엔진",
                         "한화엔진", "STX엔진", "HD현대중공업", "HD현대마린엔진",
                         # 규모
                         "역대 최대", "역대 최고", "사상 최대"]
...
BIOTECH_PIPELINE_KEYWORDS = [
    ...
    # 의료기기 CE 허가 (산업용 CE 마킹과 구분하기 위해 "의료기기" 수식어 포함)
    "의료기기 CE 인증", "의약품 CE 인증", "의료기기 유럽 허가", "EU 의약품 허가",
]
...
SUPPLY_BOTTLENECK_KEYWORDS = [
    "공급 부족", "supply shortage", "병목", "bottleneck",
    "수급 불균형", "공급 타이트", "수요 초과", "sold out",
    # 발전엔진 / 데이터센터 전력 병목
    "납기 확대", "리드타임", "lead time", "수주 적체",
    "엔진 공급", "발전엔진", "중속엔진", "4행정 엔진", "힘센엔진", "HiMSEN",
    "발전기 수주", "데이터센터 발전",
    "한화엔진", "STX엔진", "HD현대마린엔진",
    # HBM/반도체 병목 — HBM (모든 변형 포함: HBM2/3/4/3E), TC본더, CoWoS
    "HBM", "HBM3E", "HBM4", "HBM3", "HBM2E", "고대역폭 메모리",
    "HBM 수요", "HBM 공급", "HBM 패키징",
    "테스트 장비 부족", "TC본더", "TC 본더", "CoWoS",
    "핸들러 공급", "검사장비 부족", "Bonder",
    # 전력 인프라 및 열관리 병목
    "데이터센터 전력", "AI 전력 수요", "전력망 병목", "변압기 납기",
    "HVDC", "송전망", "배전망", "액체냉각", "액침냉각", "liquid cooling",
    # 열관리/냉각 세부
    "immersion cooling", "직접수냉", "냉각 솔루션", "냉각 수요",
    "GST", "케이엔솔",
    # HVDC/송전 세부
    "세명전기", "제룡산업", "가공선로", "송전선", "전력 케이블",
    # 이차전지/배터리 소재 병목 (에코프로비엠, POSCO홀딩스 등)
    "양극재 공급", "음극재 공급", "전구체 공급", "수산화리튬 공급",
    "배터리 소재 부족", "이차전지 소재", "양극재 수요", "배터리 원재료",
    "리튬 부족", "리튬 공급", "코발트 부족", "니켈 공급",
    "EV 배터리 수요", "전기차 배터리 공급", "셀 메이커 공급",
    "전고체 배터리", "전고체 소재",
    # 조선/방산 수주 병목 (HD현대중공업, 한화오션 등)
    "조선 수주 적체", "슈퍼사이클", "선가 상승", "수주잔고 증가",
    "친환경 선박 수요", "LNG 운반선 수주", "암모니아 선박",
    "납기 연장", "도크 부족", "야드 포화",
]
...
GEOPOLITICAL_KEYWORDS = ["리쇼어링", "reshoring", "국산화", "localization",
                          ...
                          # 원전/SMR 세부 (두산에너빌리티, 한전기술 등)
                          "원전", "원전 수주", "원전 수출", "SMR", "소형모듈원전",
                          "체코", "두코바니", "APR1000", "APR-1400", "APR1400",
                          "원자로", "핵연료", "원전 건설", "원전 계약",
                          # 방산 수출 세부
                          "방산", "방위산업", "K방산", "무기 수출", "방산 수출",
                          "폴란드", "루마니아", "호주", "사우디", "UAE",
                          "K-2", "K-9", "FA-50", "천무", "유도무기",
                          # 조선 탈탄소
                          "이중연료", "LNG 추진", "암모니아 추진", "친환경 선박",
                          "IMO 2030", "탈탄소 선박", "메탄올 선박",
                          ]
...
# ── 2026 신규 100배 테마 키워드 ──
# 위성/우주, 사이버보안, 양자컴퓨팅, 휴머노이드 로봇, AI 추론 칩, 디지털 자산
SPACE_KEYWORDS = [
    "위성", "satellite", "발사체", "launch vehicle", "누리호", "한화시스템 위성",
    "우주항공", "space", "저궤도", "LEO", "정지궤도", "GEO",
    "스페이스X", "SpaceX", "지구관측", "earth observation",
    "위성통신", "satellite communications", "원웹", "OneWeb",
]

CYBERSEC_KEYWORDS = [
    "사이버보안", "cybersecurity", "정보보안", "데이터 유출",
    "랜섬웨어", "ransomware", "제로트러스트", "zero trust",
    "엔드포인트 보안", "EDR", "XDR", "SOC", "SIEM",
    "보안솔루션", "침해사고", "보안 인증", "ISMS-P",
]

QUANTUM_KEYWORDS = [
    "양자컴퓨팅", "quantum computing", "양자컴", "큐비트", "qubit",
    "양자암호", "QKD", "양자통신", "양자센서",
]

HUMANOID_KEYWORDS = [
    "휴머노이드", "humanoid", "인간형 로봇", "이족보행", "bipedal",
    "옵티머스", "Optimus", "테슬라봇", "보스턴 다이내믹스", "Figure",
    "협동로봇", "cobot", "산업용 로봇",
]

AI_INFERENCE_KEYWORDS = [
    "추론 칩", "AI 추론", "inference chip", "NPU", "TPU",
    "엣지 AI", "edge AI", "AI 가속기", "AI accelerator",
    "온디바이스 AI", "on-device AI", "LPU", "Groq",
]
