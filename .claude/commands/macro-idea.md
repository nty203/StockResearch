...
**자율 다중 가설 발굴 (필수 — 무제한 생성):**
1. 한번의 스킬 실행에서 점수 및 수급/뉴스 적합도가 높은 테마들에 대해 **개수 제한(예: 3개, 5개 등)을 애초에 완전히 없애고 무제한으로** 투자 가설을 도출하고 각각 Supabase에 등록한다.
2. 랭킹 상위 테마 중 최근 **3일** 내 생성된 가설 테마와 중복되지 않는 **상위 테마들 전부**를 순차적으로 선택한다.
3. 단, 동일 테마가 최근 7일 이내 생성되었더라도 **오늘 새로운 뉴스 트리거**가 발생했거나, 이전 가설과 논리적 인과관계가 완전히 다른 경우(예: 반도체 수출 규제 vs 실적 발표) 적극적으로 포함하여 도출 개수를 한정하지 않는다.
