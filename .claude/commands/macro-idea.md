...
| Technical Alignment (수급·기술적 정렬) | 20 | **16-20**: 가설 관련 종목 중 52w 88~99% 구간 + 최근 14일 내 증권사 목표가 상향 존재 → 돌파 임박 / **10-15**: 52w 100~115% 신고가 돌파 직후(2주 이내) + 목표가 상향 → 모멘텀 시작 / **5-9**: 후보 전체가 이미 3M 100%+ 상승 → 선반영 완료, 진입 타이밍 늦음 |

**Guardrails:**
- 사후 확증 편향 금지 — 뉴스/리포트에서 Q(수요) 또는 P(가격) 실질 근거를 찾아야 함
- **조기 신호 원칙** — candidates에 이미 3개월 80%+ 오른 종목을 올릴 때는 반드시 "선반영 우려" 명시. candidates의 주력은 52w 신고가 88~99% 구간 + 목표가 상향 종목으로 구성한다.
