...
**자율 다중 가설 발굴 (필수):**
1. 한번의 스킬 실행에서 점수 및 수급/뉴스 적합도가 높은 테마들에 대해 **개수 제한 없이** 투자 가설을 도출하고 각각 Supabase에 등록한다.
2. 랭킹 상위 테마 중 최근 **3일** 내 생성된 가설 테마와 중복되지 않는 **상위 테마들**을 순차적으로 선택한다.
3. 단, 동일 테마가 최근 7일 이내 생성되었더라도 **오늘 새로운 뉴스 트리거**가 발생했거나, 이전 가설과 논리적 인과관계가 완전히 다른 경우(예: 반도체 수출 규제 vs 실적 발표) 적극적으로 포함한다.
...
print('→ 위 테마들과 겹치지 않으면서 실시간 뉴스 트리거가 발생한 상위 테마들을 선별해 가설을 각각 도출할 것')
"
```
...
선택된 각 테마 각각에 해당하는 뉴스를 필터링하여 인과관계와 Q/P 근거를 수집한다.
...
### Step 3 — 동적 후보주 검색 및 외부 데이터(주달 등) 참조

선택된 각 테마의 수혜 후보주를 **동적**으로 확보한다. 고정 바스켓에 의존하지 말고, 최신 시장 트렌드가 반영된 외부 전문 사이트 데이터를 반드시 참조한다.

```bash
# 1. 외부 전문 사이트(주달 등) 실시간 테마-관련주 검색 (필수)
cd apps/collector && uv run python -m src.search_judal "SOCAMM2"

# 2. 내부 DB(검색 엔진)와 교차 검증
cd apps/collector && uv run python -m src.search_stocks "기판" "검사장비" "패키징"
```

- **실시간 데이터 우선**: 전문 테마 사이트(주달, 인포스탁 등)에서 현재 시점에 가장 활발하게 엮이고 있는 종목 리스트를 최우선으로 확보한다.
- **논리적 필터링**: 검색된 종목 중 가설과 가장 논리적으로 잘 맞는 **3~6개**를 최종 `candidates`로 선정한다.
- **조기 신호 스코어**: `search_stocks` 스크립트가 계산해준 `early_signal_score`와 `signal_flag`를 데이터에 포함시킨다.
...
도출된 모든 가설마다 각각 Supabase `macro_ideas` 테이블에 POST 요청을 보내 개별 레코드로 저장한다.
